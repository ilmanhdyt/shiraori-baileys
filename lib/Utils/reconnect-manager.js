...
class ReconnectManager {
    ...
    shouldReconnect(lastDisconnect) {
        ...
        const isBadSession = BAD_SESSION_ERRORS.some((e) => errorMsg.includes(e)) || statusCode === 401;
        if (isBadSession) {
            return {
                reconnect: false,
        ...
        const isReconnectable = RECONNECTABLE_ERRORS.some((e) => errorMsg.includes(e)) ||
            statusCode === 408 ||
            statusCode === 503 ||
            statusCode === 500 ||
    ...
    async backoff() {
        ...
        this.logger.info({ attempt: this.state.attempts, delay, max: this.options.maxAttempts }, '[shiraori] Reconnecting with backoff...');
        await sleep(delay);
    }
